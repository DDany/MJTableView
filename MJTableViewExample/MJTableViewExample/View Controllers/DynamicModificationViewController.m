...
- (void)initTable {
    ...
    for (int i = 0; i < (self.performanceTestMode ? 99 : 1); i++) {
        ...
        // drag refresh
        MJTableRow *pulltoRefreshRow = [MJTableRow rowWithText:@"        Trigger Pull to Refresh"
                                                   detailText:nil];
        pulltoRefreshRow.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        pulltoRefreshRow.didSelectBlock = ^(NSIndexPath *indexPath){
            [self.tableView triggerPullToRefresh];
        };
        [section0.rows addObject:pulltoRefreshRow];
        ...
        MJTextViewTableRow *textViewRow = [MJTextViewTableRow rowWithText:@"MultiText Shown"
                                                              stringValue:@"ADAWDAWDASDAWDAWDABCDDAWDAWDASDAWDAWDABCDDAWDAWDASDAWDAWD"];
        ...
        textViewRow.didSelectBlock = ^(NSIndexPath *indexPath){
            ...
            row.stringValue = [row.stringValue.mutableCopy stringByAppendingString:@"AAWDADAW"];
...
- (void)initRefreshAndLoadMore {
    __weak DynamicModificationViewController *weak_self = self;
